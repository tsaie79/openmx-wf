...
def get_files(task_id, store_in, db="db.json", path="openmx_raw"):
    ...
    keys = db.collection.find_one({"task_id": task_id})["calcs_reversed"][0][path].keys()
    ...
    def write_output(key):
        ...
        print(f"---> {store_in}/{task_id}/{path}/{key} <---")
        if not os.path.exists(f"{store_in}/{task_id}/{path}"):
            os.makedirs(f"{store_in}/{task_id}/{path}")
            print(f"Directory {store_in}/{task_id}/{path} created")
        ...
        with open(f"{store_in}/{task_id}/{path}/{key}", "wb") as f:
            f.write(out)


    # Use a ThreadPoolExecutor to run the loop in parallel
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(write_output, keys)
